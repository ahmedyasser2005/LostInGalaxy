...
float4 main(PSIn psin) : SV_TARGET
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        LightDistance[i] = length(psin.lightViewPosition[i] - psin.vertexViewPosition);
    }
    ...
    for (int i = 0; i < 4; ++i)
    {
        I[i] = light[i].lightIntensity / (LightDistance[i] * LightDistance[i]);
    }
    ...
    for (int i = 0; i < 4; ++i)
    {
        float3 specular = light[i].lightTint * materialColor * I[i] * pow(max(dot(H[i], N), 0.0f), materialShininess);
        float3 diffuse = light[i].lightTint * materialColor * I[i] * max(dot(L[i], N), 0.0f);
